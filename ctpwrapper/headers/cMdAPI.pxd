...
cdef extern from 'ThostFtdcMdApi.h':
    ...
    cdef cppclass CMdSpi "CThostFtdcMdSpi":
        ...
        void SPI_OnFrontConnected "OnFrontConnected"() except +
        ...
        #  0x2002 发送心跳失败
        #  0x2003 收到错误报文
        void SPI_OnFrontDisconnected "OnFrontDisconnected"(int nReason) except +

        # 心跳超时警告。当长时间未收到报文时，该方法被调用。
        # @param nTimeLapse 距离上次接收报文的时间
        void SPI_OnHeartBeatWarning "OnHeartBeatWarning"(int nTimeLapse) except +

        # 登录请求响应
        void SPI_OnRspUserLogin "OnRspUserLogin"(CThostFtdcRspUserLoginField *pRspUserLogin,
                            CThostFtdcRspInfoField *pRspInfo,
                            int nRequestID,
                            cbool bIsLast) except +

        # 登出请求响应
        void SPI_OnRspUserLogout "OnRspUserLogout"(CThostFtdcUserLogoutField *pUserLogout,
                             CThostFtdcRspInfoField *pRspInfo,
                             int nRequestID,
                             cbool bIsLast) except +

        # 错误应答
        void SPI_OnRspError "OnRspError"(CThostFtdcRspInfoField *pRspInfo,
                        int nRequestID,
                        cbool bIsLast) except +

        # 订阅行情应答
        void SPI_OnRspSubMarketData "OnRspSubMarketData"(CThostFtdcSpecificInstrumentField *pSpecificInstrument,
                                CThostFtdcRspInfoField *pRspInfo,
                                int nRequestID,
                                cbool bIsLast) except +

        # 取消订阅行情应答
        void SPI_OnRspUnSubMarketData "OnRspUnSubMarketData"(CThostFtdcSpecificInstrumentField *pSpecificInstrument,
                                  CThostFtdcRspInfoField *pRspInfo,
                                  int nRequestID,
                                  cbool bIsLast) except +

        # 深度行情通知
        void SPI_OnRtnDepthMarketData "OnRtnDepthMarketData"(CThostFtdcDepthMarketDataField *pDepthMarketData) except +
